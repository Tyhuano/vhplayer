...
实测：typecheck 无报错；**193 用例全绿**（152 基线 + 41 新增，连跑 3 次稳定）；build 成功；ffmpeg-static 运行时 require 保留；dev 冒烟 18s 无崩溃。
...
- [x] 打开一个 m3u8 流播放 → 右键菜单「下载 MP4」可用；右上角出现下载按钮
- [x] 本地 mp4 播放时 → 右键「下载 MP4」禁用、无下载按钮
- [x] 点击下载：右下角浮层出现任务，进度随播放时长推进（百分比约等于已下载/总时长）
- [x] 未设置下载目录时文件落在系统「下载」文件夹；文件名取自流标题，重名自动加 (1)
- [x] 下载中再点一次下载（另一 m3u8）→ 第二个任务显示「等待中」，第一个完成后自动开始
- [x] 取消运行中任务 → 任务消失、输出半成品文件被删除（资源管理器确认）
- [x] 下载完成 → 任务行「已完成」+「打开目录」按钮定位文件（文件可正常播放，remux 无损）
- [x] 断网/无效源下载 → 任务失败显示错误信息、半成品被清理
- [x] 无 duration 的任务 → 进度条为不确定滚动动画
- [x] 右键「设置」→ 浮层打开：选择目录后生效（下一次下载落到新目录）；「恢复默认」回到系统下载目录；自动续播开关生效（重启后记忆）
- [x] 下载进行中退出应用 → ffmpeg 进程被杀、无残留半成品（任务管理器/文件检查）
- [x] 回归：单屏/分屏播放、收藏、列表、置顶、缩放手柄均正常；下载过程中切换视频/分屏不受影响
