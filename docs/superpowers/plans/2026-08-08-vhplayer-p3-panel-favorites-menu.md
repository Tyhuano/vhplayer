...
预期：typecheck 无报错；测试全绿（最终 107 用例，含验收后修复新增）；build 成功产出 out/。
...
验收清单（对应 spec 第 10 节）：
- [x] 右键播放区 → 自定义菜单弹出（深色圆角），倍速/模式/缩放当前项打勾，置灰项不可点击
- [x] 菜单在窗口右/下边缘自动翻转定位；点击外部/Esc 关闭
- [x] 右上「列表」按钮与 L 键开合面板，tab 记忆保留
- [x] 打开文件/文件夹/网络流 → 新列表出现在面板，下拉可切换多列表
- [x] 列表点击项播放；当前项高亮；拖拽换序生效；删除/清空生效
- [x] 排序切换：名称 / 时间正序 / 倒序（按文件修改时间，旧数据按添加时间）
- [x] 控制栏爱心：未收藏白心 → 点击变红实心；收藏 tab 出现该项；再点取消
- [x] 收藏项可点播；收藏变化关闭重开仍在（持久化）
- [x] 画面缩放 contain/fill 切换即时生效
- [x] 面板/菜单打开时播放不中断；全屏下菜单/面板正常
- [x] 回归：播放/续播/快捷键/控制列隐藏均正常
...
## 验收后修复记录（2026-08-08 用户实测反馈）

以下为验收过程中用户反馈问题的修复，均已通过测试与复验：

1. **窗口拖拽三连修**：
   - CSS `-webkit-app-region: drag` 在目标环境部分失效/与系统行为叠加 → 改为**纯 JS 拖拽**（mousedown 记录偏移 → mousemove 经 `moveTo` IPC → `win.setBounds`）
   - 根因定位：**Electron frameless 窗口 `setPosition`/`setBounds` 每次调用 +1px 尺寸漂移**（日志实证 962x543→1215x829），getSize 漂移值回传形成放大循环 → 主进程维护**期望尺寸**（仅缩放手柄/形态状态机更新），moveTo 携带固定期望值打破循环
   - `resizable: false` 消除系统顶部 8px 缩放热区；**右下角自定义缩放手柄**（`useWindowResize` + `resizeTo` IPC，min 480x320，上限工作区 1.5 倍）；拖拽/缩放状态 `mouseleave` 清理防残留
   - 顶栏右上角新增**最小化/关闭**按钮（`no-drag` 语义、关闭走保存-销毁流程）
2. **时间排序按来源区分**：文件夹/文件选择列表 `createdAt` = **文件修改时间 mtime**（主进程 stat）；文件夹列表 items 按 mtime 升序作为播放顺序；网络流/手动添加按添加时间（Date.now()）
3. **手动列表创建**：面板「＋ 新建列表」（名称输入 + √/× 确认）、「添加文件」（`media.fromPaths` 引用快照，不复制文件）、「添加流」（URL 输入独占一行）；列表名默认文件夹名、可重命名（铅笔 + √/×）；创建后「▶ 正在播放」快速返回按钮；下拉带播放中 ▶ 标记
4. **列表来源标识**：`Playlist.source`（folder/files/url/manual），下拉选项前缀「视频 · / 流 · / 列表 ·」；旧数据按首项 sourceType 回退
5. **标准图标**：全应用字符/emoji 按钮替换为 Feather Icons（MIT，可分发）内联 SVG（icons.tsx），保留 title 悬浮提示
6. **媒体链路**：新增 IPC `media:from-paths`（路径→MediaItem，主进程 stat mtime）、`window:move-to`、`window:resize-to`、`window:minimize`、`window:close`；`resizable: false`

**用例规模**：P3 结束时 107 用例全绿（12 套件）；typecheck/build 通过；零新增依赖。

---

## 验收结果与待办状态（2026-08-08）

- **P3 已验收通过**（用户确认：核心功能 + 历次反馈修复全部验证）
- **待进行**：**P4 分屏**（4 实例槽位 2x2 网格，对应父 spec 需求 11；多实例播放列表/音量/倍速独立、全局控制作用于活动格）——需先编写 P4 实现计划（writing-plans）后按阶段执行
